//! Executable QT3 `fn:upper-case` tranche.
...
#[test]
fn executes_qt3_upper_case_denominator_tranche() {
    let set_file = "fn/upper-case.xml";
    let mut selected = (1..=3)
        .map(|suffix| format!("fn-upper-case1args-{suffix}"))
        .collect::<Vec<_>>();
    selected.extend((1..=18).map(|suffix| format!("fn-upper-case-{suffix}")));
    selected.extend((20..=22).map(|suffix| format!("fn-upper-case-{suffix}")));
    selected.extend((1..=4).map(|suffix| format!("K-UpperCaseFunc-{suffix}")));
    ...
    for case_name in selected {
        ...
        let case = descendants_named(&document, document.document_node(), "test-case")
            ...
            .expect("selected QT3 upper-case case");
...
fn assert_native_result(
    ...
    let assertion = descendants_named(document, result, "assert-deep-eq")
        .into_iter()
        .next()
...
fn load_test_set(set_file: &str) -> Document {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(set_file);
    let bytes = fs::read(path).expect("read pinned QT3 upper-case test set");
    ...
    .expect("parse pinned QT3 upper-case test set");
    Document::from_parsed(parsed).expect("build pinned QT3 upper-case test set")
}
