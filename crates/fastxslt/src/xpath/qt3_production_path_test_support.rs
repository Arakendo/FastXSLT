...
use crate::runtime::golden_runtime_experiment::{
    execute_compiled_initial_template_without_source_for_test, execute_compiled_stylesheet_for_test,
};
...
pub(crate) fn compile_contextless_expression(
    case_name: &str,
    expression: &str,
) -> Result<StylesheetProgram, CompileFailure> {
    let select = expression
        ...
        .replace('\t', "&#x9;");
    let stylesheet = format!(
        r#"<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"><xsl:output method="text"/><xsl:template name="main"><xsl:value-of select="{select}"/></xsl:template></xsl:stylesheet>"#
    );
    let parsed = parse_document(
        &format!("urn:w3c:qt3:{case_name}:stylesheet"),
        stylesheet.as_bytes(),
    ...
    .expect("generated contextless stylesheet should parse");
    let document = Document::from_parsed(parsed).expect("generated stylesheet XDM should build");
    compile_stylesheet(&document)
}

pub(crate) fn execute_contextless_expression(
    program: &StylesheetProgram,
    case_name: &str,
) -> Result<String, String> {
    execute_compiled_initial_template_without_source_for_test(program, "main", case_name)
}
