//! Executable QT3 `fn:escape-html-uri` tranche.

use std::{collections::BTreeSet, fs, path::PathBuf};

use super::escape_html_uri_experiment::{EscapeHtmlUriFailure, EscapeHtmlUriValue, evaluate};
...
#[test]
fn executes_qt3_escape_html_uri_denominator_tranche() {
    let set_file = "fn/escape-html-uri.xml";
    let mut selected = (1..=6)
        .map(|suffix| format!("fn-escape-html-uri1args-{suffix}"))
        .collect::<Vec<_>>();
    selected.extend((1..=21).map(|suffix| format!("fn-escape-html-uri-{suffix}")));
    selected.extend((1..=5).map(|suffix| format!("K-EscapeHTMLURIFunc-{suffix}")));
    selected.push("cbcl-escape-html-uri-001".to_owned());
    ...
    for case_name in selected {
        ...
        let case = descendants_named(&document, document.document_node(), "test-case")
            ...
            .expect("selected QT3 escape-html-uri case");
...
fn load_test_set(set_file: &str) -> Document {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(set_file);
    let bytes = fs::read(path).expect("read pinned QT3 escape-html-uri test set");
    ...
    .expect("parse pinned QT3 escape-html-uri test set");
    Document::from_parsed(parsed).expect("build pinned QT3 escape-html-uri test set")
}
