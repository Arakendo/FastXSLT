//! Executable QT3 `fn:lower-case` tranche.
...
#[test]
fn executes_qt3_lower_case_denominator_tranche() {
    let set_file = "fn/lower-case.xml";
    let mut selected = (1..=3)
        .map(|suffix| format!("fn-lower-case1args-{suffix}"))
        .collect::<Vec<_>>();
    selected.extend((1..=18).map(|suffix| format!("fn-lower-case-{suffix}")));
    selected.extend((20..=21).map(|suffix| format!("fn-lower-case-{suffix}")));
    selected.extend((1..=4).map(|suffix| format!("K-LowerCaseFunc-{suffix}")));
    ...
    for case_name in selected {
        ...
        let case = descendants_named(&document, document.document_node(), "test-case")
            ...
            .expect("selected QT3 lower-case case");
...
fn load_test_set(set_file: &str) -> Document {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(set_file);
    let bytes = fs::read(path).expect("read pinned QT3 lower-case test set");
    ...
    .expect("parse pinned QT3 lower-case test set");
    Document::from_parsed(parsed).expect("build pinned QT3 lower-case test set")
}
