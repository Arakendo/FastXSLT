...
fn binary_numeric_paths_share_execution_with_compiled_cardinality_policy() {
    ...
    let body = r#"<xsl:output method="text"/><xsl:template match="/"><xsl:apply-templates select="doc"/></xsl:template><xsl:template match="doc"><xsl:value-of select="n1+n2"/>|<xsl:value-of select="(n1/@attrib)*(n2/@attrib)"/>|<xsl:value-of select="n-2 - n-1"/>|<xsl:value-of select="div div mod"/>|<xsl:value-of select="n-2+-n-1"/>|<xsl:value-of select="n-2 - -n-1"/>|<xsl:value-of select="-n-2 --n-1"/>|<xsl:value-of select="-n-2/@attrib --n-1/@attrib"/>|<xsl:value-of select="-(n-2/@attrib) - -(n-1/@attrib)"/>|<xsl:value-of select="n-2 mod n-1"/>|<xsl:value-of select="div mod mod"/>|<xsl:value-of select="n1*n2*n3"/>|<xsl:value-of select="n0 div n1 div n2"/>|<xsl:value-of select="(n0*n1) div n2"/></xsl:template>"#;
    let mut resources = ResourceSetBuilder::new(ResourceLimits::new(3, 8_192, 16_384));
    resources
        .admit(
            SOURCE,
    ...
    assert_eq!(
        ...
        "9|25|4|2|4|10|-4|8|8|1|0|36|2|18"
    );
