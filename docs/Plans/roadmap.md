...
1. widen the selected `path` family one semantic unit at a time, continuing
   with the arithmetic positional predicate in `path-007`, while retaining all
...
- [x] Admit all ten cases in the complete XSLT30 `path` test set and execute
  `path-001` through `path-006` through charged child, ancestor,
  ancestor-or-self, attribute, descendant-or-self, and parent predicate slices,
  retaining the other four as explicit engine-unsupported outcomes.
