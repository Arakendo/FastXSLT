...
| QT3 | Immutable submodule `83993587711dbd5c18ed846385ec37d079d6e492` | 428 test sets and 31,821 cases are structurally inventoried; 404 explicitly selected cases execute through two suite-specific XPath adapters. |
...
| **Audited subtotal** | **612** | **404** | **179** | **29** |

The 612-case subtotal is therefore conserved without relabeling the 29
...
3. [x] Give `AxisStep.xml` and `fn/deep-equal.xml` complete QT3 overlays so all
   ...
   now conserves 404 selected passes, 179 native-dependency profile exclusions,
   and 29 visible default not-run cases against the immutable upstream case
