...
| QT3 | Immutable submodule `83993587711dbd5c18ed846385ec37d079d6e492` | 428 test sets and 31,821 cases are structurally inventoried; 373 explicitly selected cases execute through two suite-specific XPath adapters. |
...
| **Audited subtotal** | **612** | **373** | **179** | **60** |

The 612-case subtotal is therefore conserved without relabeling the 60
...
3. [x] Give `AxisStep.xml` and `fn/deep-equal.xml` complete QT3 overlays so all
   ...
   now conserves 373 selected passes, 179 native-dependency profile exclusions,
   and 60 visible default not-run cases against the immutable upstream case
