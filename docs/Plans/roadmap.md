...
1. widen the selected `path` family one semantic unit at a time, continuing
   with the parent-axis predicate in `path-006`, while retaining all ten
...
- [x] Admit all ten cases in the complete XSLT30 `path` test set and execute
  `path-001` through `path-005` through charged child, ancestor,
  ancestor-or-self, attribute, and descendant-or-self predicate slices,
  retaining the other five as explicit engine-unsupported outcomes.
