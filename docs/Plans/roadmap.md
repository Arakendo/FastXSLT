...
403 passes. Static computed-element namespace URIs then raise it to 452 passes
without admitting namespace AVTs. It retains 41 executing comparison
mismatches, 16 comparator gaps, and 35
...
a strict 16.48% lower bound over the suite's 2,742 standard-operation cases,
...
| OASIS XSLT/XPath 1.0 CD04 | Hash-verified ignored local archive | All 3,173 catalog cases are measured locally; the initial 366 definite unchanged XML passes have grown to 452 after shared `xsl:element`, namespace, and node-path/node-kind `xsl:copy-of` tranches. The archive remains non-redistributed and the result is compatibility evidence, not conformance. |
...
- [x] Raise it to 452 by compiling static computed-element namespace URIs;
  retain namespace AVTs and nine newly visible mismatches as explicit work.
