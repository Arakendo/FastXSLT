...
| QT3 | Immutable submodule `83993587711dbd5c18ed846385ec37d079d6e492` | 428 test sets and 31,821 cases are structurally inventoried; 551 explicitly selected cases execute through four suite-specific XPath adapters. |
...
QT3 now has complete parent overlays for the six test sets under active
...
| **Audited subtotal** | **774** | **551** | **183** | **40** |

The 774-case subtotal is therefore conserved without relabeling the 40
unexecuted cases as engine failures. The 183 exclusions describe the current
...
composition. The other 31,047 QT3 cases remain
...
3. [x] Give `AxisStep.xml`, `fn/deep-equal.xml`, `fn/true.xml`,
   `fn/false.xml`, `fn/empty.xml`, and `fn/exists.xml` complete QT3 overlays so
   all 774 parent-set cases receive a selection disposition. The typed verifier
   now conserves 551 selected passes, 183 native-dependency profile exclusions,
   and 40 visible default not-run cases against the immutable upstream case
   identities. Explicit selection takes priority over a dependency rule. The
   two boolean-function
   sets execute 48 unchanged cases through their own
   work-accounted XPath and assertion adapter without inferring the remaining
   higher-order function-item cases. The empty-function set executes 47
   unchanged atomic-sequence cardinality and arity-error cases without
...
4. [ ] Add complete denominators deliberately, selected by standards and
   ...
   11,101 XSLT30 and 31,047 QT3 catalog-only cases must stay outside pass/fail
