...
| 5. Test-only QT3 semantics | Partially remediated 2026-09-03 | Four complete families now put 113 selected cases through compiled typed production semantics, XSLT execution, serialization, and workbench sentinels: `default-collation`, `escape-html-uri`, `encode-for-uri`, and `iri-to-uri`. Other selected test-only families remain migration debt. |
...
invoke test-only evaluators for their 113 selected cases. Each unchanged
...
four expression families through the ASP.NET workbench engine. The three URI
