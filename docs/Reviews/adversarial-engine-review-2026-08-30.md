...
| 6 | **Partially repaired -- measurement required** | Every byte outcome is now bounded before insertion with `FXFFI0014` fallback, and engine plus creation-outcome publication is all-or-nothing. [AR-0017](../Architectural%20Reviews/AR-0017-native-handle-registry-retention-and-abandonment.md) retains aggregate abandonment and prepared-engine retention for measurement before selecting count, byte, domain, or isolation policy. |
