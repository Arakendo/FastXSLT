...
Case `output-0104` adds XHTML attribute serialization at the predefined XML
namespace boundary. An `xml:lang` attribute uses the reserved `xml` prefix
without requiring a redundant namespace declaration, while an apostrophe in a
double-quoted ordinary attribute remains literal rather than becoming
`&apos;`. The admission verifies the two upstream serialization fragments and
does not generalize to HTML-method attribute conventions.

Cases `output-0107` and `output-0108` retain XML-compatible text escaping for
XHTML `script` and `style` elements. Less-than signs and ampersands therefore
remain escaped, and each complete result matches the upstream XSLT 2.0
file-backed alternative after explicit checkout-EOL normalization. This is not
the unescaped raw-text behavior of the HTML output method.

Cases `output-0114` and `output-0115` exercise the XHTML CDATA boundary with
text ending in `]]` and text containing `]]>`. The latter is serialized as two
adjacent CDATA sections so the forbidden terminator never occurs inside a
...
`application/xhtml-xml` media type. With no method declared, an
XHTML-namespaced `html` document element selects the same XHTML serializer,
retains the XML declaration, and inserts the default `text/html` metadata.
This does not weaken the explicit unsupported result for a null-namespace
`html`, which would select the unimplemented HTML method.
...
Case `output-0138` broadens CDATA evidence from declaration merging to a native
XHTML list of QNames. Unprefixed `h1` and `h5` select the stylesheet's XHTML
default namespace, while `one:h3` and `my:h3` prove that two lexical prefixes
bound to the same URI select by expanded name rather than prefix spelling. The
XHTML `h2`, XHTML `h3`, XHTML `h4`, and same-local-name `h3` in a different
default namespace retain ordinary escaped text. Bounded literal-fragment checks
cover every member of the upstream `all-of`; this does not admit a general
regular-expression comparator.

Case `output-0153` retains the explicitly requested XML serialization version
`1.0` in compiled output state and emits it in the XML-compatible XHTML
declaration. Other versions fail explicitly as unsupported `FXST1021`; the case
does not claim XML 1.1 output or XHTML version/DOCTYPE rules beyond its complete
admitted result. Case `output-0156` independently composes XML output with
`include-content-type="no"` and proves the XHTML-only metadata remains inert:
the exact result contains only the XML declaration and namespace-qualified
`out` element.
...
resolution by the harness, bounded memory admission, and sixty-three exact or
...
unsupported frontier for the other 169
