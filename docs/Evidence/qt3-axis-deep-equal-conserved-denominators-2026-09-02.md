...
| **Conserved total** | **612** | **367** | **179** | **66** |
...
263 unique case names, proves all 367 selected identities exist in the correct
parent set, reads native per-case dependency metadata, and proves the remaining
179 profile exclusions plus 66 visible defaults. The private ledger is also
checked to contain exactly those 367 cases and no third test-set family.
...
This change does not convert the 66 defaults into engine failures, assert that
...
Seven literal-array cases establish the first bounded XDM-array comparison
slice while retaining member-sequence boundaries.
