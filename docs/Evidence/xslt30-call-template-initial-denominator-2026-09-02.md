...
- Unchanged case `call-template-0001`, `call-template-0101` through
  ...
  `call-template-0109`, `call-template-0201`, `call-template-0401a`,
  `call-template-0402`, `call-template-0801`,
...
`call-template-0402` evaluates the relative child path `a` at the call site and
passes the resulting source-node sequence without losing node identity. The
callee obtains its string value through the invocation-local node frame. Its
`match`, `mode`, and `priority` attributes do not participate in named-template
selection.
...
Adding this denominator changes conserved XSLT30 accounting to 675 cases: 460
passed comparisons, 3 engine-unsupported cases, 55 profile exclusions, and 157
visible default not-run cases across 17 complete test-set denominators.

## Limitation

This evidence does not admit the other 21 cases. Catalog QName resolution is
...
general XPath arguments or arbitrary node-sequence construction, deep/tail recursion
