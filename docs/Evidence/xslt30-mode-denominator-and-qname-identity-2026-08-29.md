...
- Selected cases: `mode-0105`, `mode-0106`
...
overlay's default `harness-unsupported / not-run` disposition. Exactly two
...
removes one independent blocker from `conflict-resolution-1401` without
claiming that case: its global temporary-tree path, union match pattern, text
nodes, and temporary-focus `xsl:next-match` still require dedicated work.

## Claim boundary

Two passes out of 169 are not a mode-conformance percentage. This slice does
